...
- `skills/`: reusable Codex skills. `learning-coach` is topic-neutral; a topic may add a focused skill such as `cmake-coach`.
...
4. Add a topic skill under `skills/<topic>-coach/` when the topic needs domain-specific workflow, references, or tools. Keep the shared `learning-coach` generic.
...
1. Inspect `git status` and stage only source, `topics/`, `learning/`, `skills/`, and relevant repository guidance.
